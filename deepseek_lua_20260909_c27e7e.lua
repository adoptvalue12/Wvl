--[[
    Yuno Value – Adopt Me Value Checker & Trade Evaluator
    Version mobile-friendly avec barre de titre + onglets intégrés
    Toutes les valeurs proviennent des captures d'écran
]]
...
-- ============================================================================
-- CRÉATION DE L'INTERFACE MOBILE
-- ============================================================================
local function createMainGUI()
    local screenGui = Instance.new("ScreenGui")
    screenGui.Name = "YunoValueChecker"
    screenGui.ResetOnSpawn = false
    screenGui.Parent = player.PlayerGui

    -- ===== FENÊTRE PRINCIPALE (dimensions adaptées) =====
    local mainFrame = Instance.new("Frame")
    mainFrame.Name = "MainFrame"
    mainFrame.Size = UDim2.new(0, math.min(460, guiService:GetScreenSize().X * 0.92), 0, math.min(640, guiService:GetScreenSize().Y * 0.85))
    ...
    header.Name = "Header"
    header.Size = UDim2.new(1, 0, 0, 90)  -- 50 pour titre + 40 pour onglets
    ...
    -- Sous-fond pour masquer les coins inférieurs (car on veut que seuls les coins supérieurs soient arrondis)
    ...
    searchBox.PlaceholderText = "Cherche un pet..."
    ...
    -- Image du pet (placeholder)
    local petImg = Instance.new("ImageLabel")
    petImg.Name = "PetImage"
    ...
    notFound.Position = UDim2.new(0, 12, 0, 140)
    notFound.BackgroundTransparency = 1
    notFound.Text = "⚠️ Pet inconnu"
    ...
    local function performSearch(query)
        ...
        if value then
            ...
            notFound.Visible = false
            resultFrame:TweenSize(UDim2.new(1, -24, 0, 180), "Out", "Quad", 0.15, true)
        else
            resultFrame.Visible = false
    ...
    favBtn.MouseButton1Click:Connect(function()
        if currentPetName ~= "" and not table.find(favorites, currentPetName) then
    ...
    for i, theme in ipairs(themes) do
        ...
        btn.MouseButton1Click:Connect(function()
            ...
            tabRow.BackgroundColor3 = theme.bar  -- fond des onglets
            contentArea.BackgroundColor3 = theme.bg
            -- Mettre à jour les onglets actifs
            for j, tb in ipairs(tabButtons) do
                if j == 1 then -- onglet actif par défaut
    ...
    -- ===== DRAG (optimisé mobile) =====
    -- On attrape le header complet, mais on exclut les boutons (min, close, tabs)
    local dragData = {dragging = false, startPos = nil, framePos = nil}
    local function isButton(target)
        -- Vérifie si la cible est un bouton ou un descendant d'un bouton
        while target and target ~= header do
    ...
    local function onInputBegan(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            -- Ne pas démarrer le drag si on a cliqué sur un bouton ou une zone de saisie
            local target = input.UserInputType == Enum.UserInputType.Touch and guiService:GetTouchObject(input) or nil
            -- On vérifie via le hit de l'écran
            local hit = userInputService:GetMouseLocation()
            -- Mais on va plutôt utiliser l'événement InputBegan avec le target dans les paramètres
            -- Malheureusement, InputBegan ne donne pas le target directement, on doit utiliser un autre moyen.
            -- On va plutôt détecter si on a cliqué sur un bouton via le hit test
            -- Mais c'est complexe. Une approche plus simple : on attrape le drag sur le header, mais on vérifie si le clic est sur un enfant de type TextButton.
            -- On peut utiliser MouseButton1Down sur le header mais on perd le toucher.
            -- On va plutôt utiliser le toucher sur le header et ignorer si le touch a commencé sur un bouton.
            -- On peut déterminer le target en utilisant GetMouseLocation et en faisant un point dans l'écran.
            -- Pour plus de simplicité, on va utiliser les événements MouseButton1Down / TouchStart sur le header, mais on vérifie si le touch a commencé sur un bouton en utilisant GetChildren?
            -- En fait, on peut simplement ne pas démarrer le drag si le input a été déclenché par un bouton en utilisant la propriété 'UserInputState' et en vérifiant le 'Target' via l'événement ? Pas possible directement.
            -- Alternative : on peut mettre un draggable frame sur le header, mais le mettre en arrière-plan, et les boutons par-dessus. Le drag ne se déclenchera que si on touche la zone vide.
            -- C'est plus simple : on crée un frame transparent qui couvre le header mais qui est en dessous des boutons. On met le drag sur ce frame.
            -- On va donc créer un frame DragArea dans le header, avec un ZIndex bas, et on met le drag sur lui.
            -- On va faire ça dans la construction.
        end
    end

    -- On va plutôt créer une zone de drag séparée (un frame transparent) dans le header, en dessous des boutons.
    -- On place ce frame dans header, size = header, mais ZIndex = 0, et on met les boutons avec ZIndex = 1.
    -- On s'assure que les boutons sont au-dessus.
    -- On attache les événements InputBegan/Ended/Movement sur ce frame.
    ...
    -- On repositionne les enfants du header pour qu'ils soient au-dessus (ZIndex > 0)
    ...
    -- Drag logic on dragArea
    local dragData2 = {dragging = false, startPos = nil, framePos = nil}
    local function onDragStart(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            dragData2.dragging = true
            dragData2.startPos = input.Position
            dragData2.framePos = mainFrame.Position
        end
    end
    local function onDragEnd(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            dragData2.dragging = false
        end
    end
    local function onDragMove(input)
        if dragData2.dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
            local delta = input.Position - dragData2.startPos
            mainFrame.Position = UDim2.new(dragData2.framePos.X.Scale, dragData2.framePos.X.Offset + delta.X,
                                           dragData2.framePos.Y.Scale, dragData2.framePos.Y.Offset + delta.Y)
        end
    end

    dragArea.InputBegan:Connect(onDragStart)
    dragArea.InputEnded:Connect(onDragEnd)
...
print("✅ Yuno Value (mobile) chargé ! " .. #petValues .. " pets en base.")