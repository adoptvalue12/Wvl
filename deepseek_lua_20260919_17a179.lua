-- Yuno Hub by Halil Tasty | Steal an Egg — Obsidian UI Edition
-- 6 scripts merged: Oxide, FS StealAnEgg, Yomogi/Exe Pet Spawner, Ragdoll Bad TP, Clao ServerHop

pcall(function() if _G.YunoHub and type(_G.YunoHub.Unload) == "function" then _G.YunoHub.Unload() end end)
...
local function preciseWait(s) if not s or s <= 0 then return end local e = 0 while e < s and not HUB.dead do e = e + RunService.Heartbeat:Wait() end end
...
do
    ...
    pcall(function()
        if type(getcustomasset) == "function" and type(writefile) == "function" then
            if type(isfile) ~= "function" or not isfile("yuno_bg.jpeg") then
    ...
    if not bgGui.Parent then pcall(function() bgGui.Parent = LP:WaitForChild("PlayerGui", 5) end) end
...
-- ============ Anti-Slide / Anti-Ragdoll / Anti-Knockback ============
...
local function applyAntiRagdollState(character, humanoid)
...
local function applyZombieDesync(character)
    if not character then return end
    local originalHumanoid = character:WaitForChild("Humanoid", 5)
    ...
    originalHumanoid.BreakJointsOnDeath = false
    local clonedHumanoid = originalHumanoid:Clone()
    clonedHumanoid.Name = "Humanoid"
    pcall(function() clonedHumanoid:SetStateEnabled(Enum.HumanoidStateType.Dead, false) end)
    clonedHumanoid.Health = originalHumanoid.MaxHealth
    local animator = clonedHumanoid:FindFirstChildOfClass("Animator")
    if not animator then animator = Instance.new("Animator") animator.Parent = clonedHumanoid end
    pcall(function()
        local jumpAnim = Instance.new("Animation")
        jumpAnim.AnimationId = "rbxassetid://125750702"
        local jumpTrack = animator:LoadAnimation(jumpAnim)
        clonedHumanoid.Jumping:Connect(function(isActive) if isActive then jumpTrack:Play() end end)
    end)
    originalHumanoid:Destroy()
    clonedHumanoid.Parent = character
    activeHumanoid = clonedHumanoid
    applyAntiRagdollState(character, clonedHumanoid)
    local camera = Workspace.CurrentCamera
    if camera then camera.CameraSubject = clonedHumanoid end
...
track(LP.CharacterAdded:Connect(function(c) task.wait(0.2) enforceGrip(c) setupCharacter(c) task.spawn(function() pcall(applyZombieDesync, c) end) end))
if LP.Character then task.spawn(function() pcall(applyZombieDesync, LP.Character) end) end

-- ============ Instant Prompt / Debris / Traps ============
pcall(function()
...
track(Workspace.DescendantAdded:Connect(function(d)
    if avoidTrapsEnabled and d.Name == "PlayerTrap" then
        task.wait(0.05)
        if d:GetAttribute("Owner") ~= LP.Name then
            if d:IsA("BasePart") then d.CanTouch = false end
            for _, c in ipairs(d:GetChildren()) do if c:IsA("BasePart") then c.CanTouch = false end end
        end
    end
end))

-- ============ L-Pattern Walk / Controls Blocking ============
    ...
    if playerScripts then
        for _, v in ipairs(playerScripts:GetDescendants()) do
            if v:IsA("LocalScript") or v:IsA("ModuleScript") then
                ...
                if n:find("shake") or n:find("camshake") or n:find("recoil") then pcall(function() v.Disabled = true end) end
            end
        end
    end
...
local function cleanTargetElement(instance)
    ...
    elseif instance:IsA("Script") or instance:IsA("LocalScript") then
        local n = instance.Name:lower()
        if n:find("anim") or n:find("pet") or n:find("float") or n:find("bob") or n:find("rotate") or n:find("spin") or n:find("water") or n:find("river") then pcall(function() instance.Disabled = true end) end
    end
end
...
local function hopOnce(threshold)
    ...
    if not serverId then return false, "No server found" end
...
local BaseTab = Window:AddTab('Base')
local CombatTab = Window:AddTab('Combat')
local PlayerTab = Window:AddTab('Player')
local TravelTab = Window:AddTab('Travel')
local HookTab = Window:AddTab('Auto Hook')
...
-- ============ MAIN TAB ============
local MainInfo = MainTab:AddLeftGroupbox('Info')
MainInfo:AddLabel('Yuno Hub by Halil Tasty')
MainInfo:AddLabel('6 scripts merged: Oxide, FS,')
MainInfo:AddLabel('Yomogi/Exe Pet Spawner,')
MainInfo:AddLabel('Ragdoll Bad TP, Clao ServerHop')

local MainUtil = MainTab:AddRightGroupbox('Utilities')
MainUtil:AddButton('Fullbright ON', function() setFullbright(true) Library:Notify({Title="Visual", Description="Fullbright ON", Time=2}) end)
MainUtil:AddButton('Fullbright OFF', function() setFullbright(false) Library:Notify({Title="Visual", Description="Fullbright OFF", Time=2}) end)
MainUtil:AddButton('Delete Own Pet Renders', function()
    local c = DeleteOwnPetRenders()
    Library:Notify({Title="Perf", Description=c .. " removed", Time=2})
end)
MainUtil:AddButton('Claim All Rewards Now', function()
    ClaimAllAvailableRewards()
    Library:Notify({Title="Rewards", Description="Claimed", Time=2})
end)

-- ============ EGGS TAB ============
local EggsLeft = EggsTab:AddLeftGroupbox('Auto Steal')
EggsLeft:AddToggle('steal_auto', {
    Text = 'Auto Steal Eggs',
    Default = false,
    Callback = function(v)
        autoStealEnabled = v
        if v then EnsureSavedReturnPosition() end
        Library:Notify({Title="Auto Steal", Description=v and "Enabled" or "Disabled", Time=2})
    end
})
EggsLeft:AddDropdown('steal_method', {
...
EggsLeft:AddSlider('glide_speed', {
    Text = 'Glide Speed',
    Default = 750, Min = 50, Max = 750,
    Callback = function(v) glideSpeed = v end
})
EggsLeft:AddSlider('steal_gap', {
    Text = 'Steal Delay (s)',
    Default = 1.5, Min = 0.5, Max = 10,
    Callback = function(v) stealDelay = v end
})
EggsLeft:AddDropdown('steal_rarities', {
    Text = 'Filter Rarity',
    Values = RARITY_NAMES,
    Multi = true,
    Default = {},
    Callback = function(sel)
        ...
        if type(sel) == "table" then
            for k, v in pairs(sel) do
                if v == true then selectedStealRarities[k] = true
                elseif type(v) == "string" then selectedStealRarities[v] = true end
            end
        end
    end
})
EggsLeft:AddDropdown('steal_areas', {
    Text = 'Filter Area',
    Values = AREA_NAMES,
    Multi = true,
    Default = {},
    Callback = function(sel)
        ...
        if type(sel) == "table" then
            for k, v in pairs(sel) do
                if v == true then selectedStealAreas[k] = true
                elseif type(v) == "string" then selectedStealAreas[v] = true end
            end
        end
    end
})
EggsLeft:AddDropdown('steal_muts', {
    ...
    Callback = function(sel)
        ...
        if type(sel) == "table" then
            for k, v in pairs(sel) do
                if v == true then selectedMutationTypes[k] = true
                elseif type(v) == "string" then selectedMutationTypes[v] = true end
            end
        end
    end
})
EggsLeft:AddToggle('steal_parasite_only', {
    Text = 'Parasite Eggs Only',
    Default = false,
    Callback = function(v) stealParasiteOnly = v end
})
EggsLeft:AddToggle('steal_big_only', {
    Text = 'Big Eggs Only',
    Default = false,
    Callback = function(v) stealBigEggsOnly = v end
})
EggsLeft:AddButton('Steal Best Egg (1x)', function()
    local eggs = GetMatchingFieldEggs()
    if #eggs == 0 then
        Library:Notify({Title="Steal", Description="No matching egg", Time=2})
        return
    end
    local ok = StealSpecificEggRobust(eggs[1])
    Library:Notify({Title="Steal", Description=ok and "Egg stolen" or "Failed", Time=2})
end)

local EggsRight = EggsTab:AddRightGroupbox('Hatch & Plant')
EggsRight:AddToggle('hatch_auto', {
    Text = 'Auto Hatch Ready Eggs',
    Default = false,
    Callback = function(v) autoHatchEnabled = v end
})
EggsRight:AddToggle('plant_auto', {
    Text = 'Auto Plant in Pen',
    Default = false,
    Callback = function(v) autoPlantEnabled = v end
})
EggsRight:AddSlider('hatch_gap', {
    Text = 'Hatch Check Delay (s)',
    Default = 2.0, Min = 0.5, Max = 10,
    Callback = function(v) hatchDelay = v end
})
EggsRight:AddButton('Hatch All Ready Now', function()
    local c = HatchAllReadyEggs()
    Library:Notify({Title="Hatch", Description=c .. " hatched", Time=2})
end)
EggsRight:AddButton('Plant All In Pen Now', function()
    local c = PlantAllCarriedEggsInPen()
    Library:Notify({Title="Plant", Description=c .. " planted", Time=2})
end)

-- ============ PETS TAB ============
local PetsLeft = PetsTab:AddLeftGroupbox('Pet Spawner')
PetsLeft:AddDropdown('pet_mutation', {
    Text = 'Mutation',
    Values = { "None", "Golden", "Rainbow", "Silver", "Sakura", "GreatBloom" },
    Default = "None",
    Multi = false,
    Callback = function(v) currentMutation = v end
})
PetsLeft:AddDropdown('pet_size', {
    Text = 'Size',
    Values = { "1x", "2x", "5x", "10x", "25x" },
    Default = "1x",
    Multi = false,
    Callback = function(v) currentSize = tonumber(tostring(v):match("%d+")) or 1 end
})
PetsLeft:AddToggle('pet_auto_equip', {
    Text = 'Auto Equip Spawned',
    Default = true,
    Callback = function(v) petAutoEquip = v end
})
PetsLeft:AddButton('Clear Spawned Pets', function()
    clearAllSpawnedPets()
    Library:Notify({Title="Pets", Description="Cleared", Time=2})
end)
if #CATALOG > 0 then
    local petNames = {}
    for _, p in ipairs(CATALOG) do table.insert(petNames, p.display) end
    PetsLeft:AddDropdown('pet_pick', {
        Text = 'Spawn Pet',
        Values = petNames,
        Default = petNames[1],
        Multi = false,
        Callback = function(sel)
            for _, p in ipairs(CATALOG) do
                if p.display == tostring(sel) then
                    task.spawn(function() doSpawnPet(p) end)
                    break
                end
            end
        end
...
local PetsRight = PetsTab:AddRightGroupbox('Base / Rewards')
PetsRight:AddToggle('up_base', { Text = 'Auto Upgrade Base', Default = false, Callback = function(v) autoUpgradeBase = v end })
PetsRight:AddToggle('up_tread', { Text = 'Auto Upgrade Treadmill', Default = false, Callback = function(v) autoUpgradeTreadmill = v end })
PetsRight:AddToggle('equip_best', { Text = 'Auto Equip Best Pets', Default = false, Callback = function(v) autoEquipBestPets = v end })
PetsRight:AddToggle('claim_rewards', { Text = 'Auto Claim Rewards', Default = false, Callback = function(v) autoClaimRewards = v end })
PetsRight:AddToggle('buy_trails', { Text = 'Auto Buy Trails', Default = false, Callback = function(v) autoBuyTrails = v end })
PetsRight:AddToggle('sell_low', { Text = 'Auto Sell Low-Tier Pets', Default = false, Callback = function(v) autoSellPets = v end })

-- ============ BASE TAB ============
local BaseLeft = BaseTab:AddLeftGroupbox('Base Actions')
BaseLeft:AddButton('Upgrade Base Now', function() UpgradeHomesteadBase() Library:Notify({Title="Base", Description="Requested", Time=2}) end)
BaseLeft:AddButton('Upgrade Treadmill Now', function() UpgradeTreadmillTier() Library:Notify({Title="Treadmill", Description="Requested", Time=2}) end)
BaseLeft:AddButton('Equip Best Pets Now', function() EquipBestPets() Library:Notify({Title="Pets", Description="Equipped", Time=2}) end)
BaseLeft:AddButton('Buy All Affordable Trails', function() BuyAffordableTrails() Library:Notify({Title="Trails", Description="Purchased", Time=2}) end)

local BaseRight = BaseTab:AddRightGroupbox('Rewards & Monster')
BaseRight:AddButton('Claim All Rewards Now', function() ClaimAllAvailableRewards() Library:Notify({Title="Rewards", Description="Claimed", Time=2}) end)
BaseRight:AddButton('Claim Monster Chest Now', function() ClaimMonsterChests() Library:Notify({Title="Monster", Description="Claimed", Time=2}) end)
BaseRight:AddButton('Feed Monster Parasite Now', function() FeedMonsterParasite() Library:Notify({Title="Monster", Description="Fed", Time=2}) end)
BaseRight:AddButton('Sell Low-Tier Pets Now', function() SellSelectedPets(nil) Library:Notify({Title="Sell", Description="Sold", Time=2}) end)
BaseRight:AddButton('Sell Low-Tier Eggs Now', function() SellSelectedEggs() Library:Notify({Title="Sell", Description="Sold", Time=2}) end)

-- ============ COMBAT TAB ============
local CombatLeft = CombatTab:AddLeftGroupbox('Bat Aura')
CombatLeft:AddToggle('bat_aura', { Text = 'Bat Aura', Default = false, Callback = function(v) batAuraEnabled = v end })
CombatLeft:AddSlider('bat_radius', { Text = 'Aura Radius', Default = 20, Min = 5, Max = 50, Callback = function(v) batAuraRadius = v end })
CombatLeft:AddSlider('bat_delay', { Text = 'Swing Delay', Default = 0.2, Min = 0.05, Max = 1, Callback = function(v) batAuraDelay = v end })
CombatLeft:AddButton('Swing Bat Once', function()
    local re = GetNetRemote("RE/BatSwing/Trigger")
    if re then pcall(function() re:FireServer() end) end
    Library:Notify({Title="Bat", Description="Swing", Time=2})
end)

local CombatRight = CombatTab:AddRightGroupbox('Defense')
CombatRight:AddToggle('anti_ragdoll', {
    Text = 'Anti-Ragdoll',
    Default = true,
    Callback = function(v) antiRagdollEnabled = v end
})
CombatRight:AddToggle('anti_slide', {
    Text = 'Anti-Slide (High Grip)',
    Default = true,
    Callback = function(v)
        antiSlideEnabled = v
        if v and LP.Character then enforceGrip(LP.Character) end
    end
})
CombatRight:AddToggle('anti_kb', {
    Text = 'Anti-Knockback',
    Default = true,
    Callback = function(v) antiKnockbackEnabled = v end
})
CombatRight:AddToggle('avoid_traps', {
    Text = 'Anti-Trap',
    Default = true,
    Callback = function(v)
        avoidTrapsEnabled = v
...
-- ============ PLAYER TAB ============
local PlayerLeft = PlayerTab:AddLeftGroupbox('Movement')
PlayerLeft:AddToggle('ws_enabled', {
...
PlayerLeft:AddSlider('ws_val', {
    Text = 'WalkSpeed',
    Default = 24, Min = 16, Max = 1000,
    Callback = function(v) walkSpeedVal = v end
})
PlayerLeft:AddToggle('jp_enabled', { Text = 'Enable JumpPower', Default = false, Callback = function(v) jumpPowerEnabled = v end })
PlayerLeft:AddSlider('jp_val', {
    Text = 'JumpPower',
    Default = 60, Min = 50, Max = 300,
    Callback = function(v) jumpPowerVal = v end
})
PlayerLeft:AddToggle('inf_jump', { Text = 'Infinite Jump', Default = false, Callback = function(v) infiniteJump = v end })
...
PlayerLeft:AddSlider('fly_speed', {
    Text = 'Fly Speed',
    Default = 60, Min = 20, Max = 250,
    Callback = function(v) flySpeed = v end
})
PlayerLeft:AddToggle('anti_afk', { Text = 'Anti-AFK', Default = false, Callback = function(v) setAntiAFK(v) end })

-- ============ TRAVEL TAB ============
local TravelLeft = TravelTab:AddLeftGroupbox('Area Travel')
local areaKeys = {}
for k in pairs(AREA_COORDINATES) do table.insert(areaKeys, k) end
table.sort(areaKeys)
local selectedAreaTP = "Base / Plot"
TravelLeft:AddDropdown('tp_area', {
    Text = 'Select Area',
    Values = areaKeys,
    Default = "Base / Plot",
    Multi = false,
...
TravelLeft:AddButton('Travel to Selected Area', function()
    local pos = AREA_COORDINATES[selectedAreaTP]
    if pos then
        Library:Notify({Title="Travel", Description="To " .. tostring(selectedAreaTP), Time=2})
        TravelRoadPath(pos, glideSpeed)
        Library:Notify({Title="Travel", Description="Arrived", Time=2})
    end
end)

local TravelRight = TravelTab:AddRightGroupbox('Plot & Player Travel')
local plotOptions = { "Plot 1","Plot 2","Plot 3","Plot 4","Plot 5","Plot 6","Plot 7","My Plot" }
local selectedPlotNum = "My Plot"
TravelRight:AddDropdown('tp_plot', {
    Text = 'Select Plot',
    Values = plotOptions,
    Default = "My Plot",
    Multi = false,
...
TravelRight:AddButton('Travel to Plot', function()
    local slotNum = selectedPlotNum == "My Plot" and 1 or tonumber(tostring(selectedPlotNum):match("%d+")) or 1
    ...
    if plot then
        ...
        Library:Notify({Title="Plot", Description="Arrived Plot " .. slotNum, Time=2})
    end
end)
...
local pDropdown
pDropdown = TravelRight:AddDropdown('tp_plr', {
    Text = 'Select Player',
    Values = getPlayerList(),
    Multi = false,
    Callback = function(v) selectedPlayerName = v end
})
TravelRight:AddButton('Refresh Player List', function()
    if pDropdown and pDropdown.Refresh then
        pcall(function() pDropdown:Refresh(getPlayerList()) end)
    end
    Library:Notify({Title="Players", Description="Refreshed", Time=2})
end)
TravelRight:AddButton('Travel to Player', function()
    ...
    if hrp then
        TravelRoadPath(hrp.Position + Vector3.new(0, 2, 0), glideSpeed)
        Library:Notify({Title="Travel", Description="Arrived at " .. selectedPlayerName, Time=2})
    end
end)

local TravelWP = TravelTab:AddLeftGroupbox('Waypoints')
local wpInput = "Waypoint"
TravelWP:AddInput('wp_name', {
    Text = 'Waypoint Name',
    Default = "Waypoint",
    Placeholder = "Name",
    Callback = function(v) if v and v ~= "" then wpInput = v end end
})
TravelWP:AddButton('Save Current Waypoint', function()
    local r = findHRP()
    if r then
        addWaypoint(wpInput ~= "" and wpInput or ("WP " .. (#waypoints + 1)), r.CFrame, r.Position)
        Library:Notify({Title="Waypoint", Description="Saved: " .. wpInput, Time=2})
    end
end)

-- ============ AUTO HOOK TAB ============
local HookLeft = HookTab:AddLeftGroupbox('Auto Hook')
HookLeft:AddToggle('hook_v1', {
    Text = 'Auto Hook v1 (Base drop)',
    Default = false,
    Callback = function(v)
        hookV1Enabled = v
        if v then hookV2Enabled = false end
    end
})
HookLeft:AddToggle('hook_v2', {
    Text = 'Auto Hook v2 (Forest drop)',
    Default = false,
    Callback = function(v)
        hookV2Enabled = v
        if v then hookV1Enabled = false end
    end
})
HookLeft:AddSlider('hook_delay', {
    Text = 'Hook Delay (s)',
    Default = 0.1, Min = 0, Max = 1,
    Callback = function(v) HookDelay = v end
})
HookLeft:AddSlider('drop_delay', {
    Text = 'Drop Egg Delay (s)',
    Default = 0.2, Min = 0, Max = 1,
    Callback = function(v) DropEggDelay = v end
})

-- ============ ESP TAB ============
local EspLeft = EspTab:AddLeftGroupbox('Egg ESP')
EspLeft:AddToggle('esp_eggs', {
    Text = 'Egg ESP Enabled',
    Default = false,
    Callback = function(v) esp.enabled = v end
})
EspLeft:AddToggle('esp_pet_icons', {
    Text = 'Show Pet Icon Badges',
    Default = true,
    Callback = function(v) esp.showPetIcons = v end
})
EspLeft:AddToggle('esp_rare_only', {
    Text = 'Rare Eggs Only',
    Default = false,
    Callback = function(v) esp.rareEggsOnly = v end
})
EspLeft:AddSlider('esp_dist', {
    Text = 'Max Distance',
    Default = 800, Min = 100, Max = 2500,
    Callback = function(v) esp.maxDistance = v end
})
EspLeft:AddToggle('esp_traps', {
    Text = 'Trap ESP',
    Default = false,
    Callback = function(v) esp.traps = v end
})
EspLeft:AddToggle('esp_players', {
    Text = 'Player ESP',
    Default = false,
...
-- ============ SERVER TAB ============
local ServerLeft = ServerTab:AddLeftGroupbox('Server Hop')
local hopThreshold = 1
ServerLeft:AddInput('hop_max', {
    Text = 'Max Players',
    Default = "1",
    Placeholder = "1",
    Callback = function(v) hopThreshold = tonumber(v) or 1 end
})
ServerLeft:AddButton('Hop Now', function()
    local ok, err = hopOnce(hopThreshold)
    Library:Notify({Title="Hop", Description=ok and "Teleporting..." or ("Failed: " .. tostring(err)), Time=3})
end)
ServerLeft:AddButton('Find Low Server', function()
    task.spawn(function()
        local target = findLowServer()
        if target then
            Library:Notify({Title="Server", Description="Joining low server...", Time=3})
            pcall(function() TeleportService:TeleportToPlaceInstance(game.PlaceId, target.id, LP) end)
        else
            Library:Notify({Title="Server", Description="No server found", Time=3})
        end
    end)
end)
ServerLeft:AddToggle('hop_auto', {
    Text = 'Auto Hop',
    Default = false,
    Callback = function(v)
        hopAutoEnabled = v
        if v then
            if hopAutoThread then pcall(task.cancel, hopAutoThread) end
            hopAutoThread = task.spawn(function()
                while hopAutoEnabled and not HUB.dead do
                    local c = #Players:GetPlayers()
                    if c <= hopThreshold then
                        task.wait(3)
                    else
                        hopOnce(hopThreshold)
                        task.wait(5)
                    end
                end
            end)
...
-- ============ PERFORMANCE TAB ============
local PerfLeft = PerfTab:AddLeftGroupbox('FPS')
PerfLeft:AddToggle('fps_boost', {
    Text = 'FPS Booster',
    Default = false,
    Callback = function(v) toggleFpsBooster(v) end
})
PerfLeft:AddButton('Delete Own Pet Renders', function()
    local c = DeleteOwnPetRenders()
    Library:Notify({Title="Perf", Description=c .. " removed", Time=2})
end)
PerfLeft:AddButton('Fullbright ON', function() setFullbright(true) Library:Notify({Title="Visual", Description="ON", Time=2}) end)
PerfLeft:AddButton('Fullbright OFF', function() setFullbright(false) Library:Notify({Title="Visual", Description="OFF", Time=2}) end)
...
HUB.Unload = function()
    ...
    pcall(stopCurrentWalking, false)
    ...
    pcall(function() if HUB.bgGui then HUB.bgGui:Destroy() end end)
    _G.YunoHub = nil
end
...
SaveManager:SetFolder('YunoHub')
ThemeManager:SetFolder('YunoHub')
SaveManager:BuildConfigSection(UISettingsTab)
ThemeManager:ApplyToTab(UISettingsTab)

Library:Notify({
    Title = "Yuno Hub",
    Description = "by Halil Tasty — loaded",
    Time = 4
})