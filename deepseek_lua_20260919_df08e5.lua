...
local Window = Library:CreateWindow({
    Title = 'Yuno Hub',
    Footer = 'by Halil The Tasty Crousty',
    Icon = 'crown',
    NotifySide = 'Right',
...
local function Notify(title, content, kind, dur)
    pcall(function()
        Library:Notify({ Title = title, Description = content, Time = dur or 2.5 })
    end)
end
...
local function safeCallback(fn)
    return function(...)
        ...
        if not ok then pcall(Notify, "Yuno Hub", "Error: " .. tostring(err), "Error", 4) end
    end
end
...
local function bypassClientDetections()
    ...
    for _, tbl in pairs(ups) do
        if typeof(tbl) == "table" then
            if pcall(setMeta, tbl, { __newindex = function() end }) then blocked = blocked + 1 end
        end
    end
    return blocked > 0
end
...
task.spawn(function()
    while not HUB.dead do
        ...
        if not alive then
            table.clear(remoteSet); anyRemote = nil; model = nil
            scanRemotes()
        end
    end
end)
    ...
    if char then
        for _, t in ipairs(char:GetChildren()) do
            ...
            if t:IsA("Tool") then
                ...
                if tName:find("egg") or (tName ~= "bat" and tName ~= "defaulttool" and not tName:find("bat") and not tName:find("slap") and not tName:find("coil") and not tName:find("potion") and not tName:find("lantern")) then return true end
            end
        end
    end
...
local function StealSpecificEggRobust(targetItem)
    ...
    local function fireInstantNow()
        ...
        instantFired = true; carried = true
        task.spawn(function()
    ...
    while os.clock() - tPickup < maxWait and not HUB.dead do
        ...
        if isInstantTP then task.wait() else task.wait(0.08) end
    end
...
local function GetMyMonsterPosition()
    ...
    if myMonster then
        return (myMonster.PrimaryPart and myMonster.PrimaryPart.Position) or myMonster:GetPivot().Position
    end
            ...
            if hrp then
                ...
                for _, p in ipairs(Players:GetPlayers()) do
                    if p ~= LP and p.Character then
                        ...
                        if oHrp and (oHrp.Position - hrp.Position).Magnitude <= batAuraRadius then foundNearby = true; break end
                    end
                end
...
-- ====== EGGS ======
local StealGB   = EggsTab:AddLeftGroupbox('Auto Steal')
local HatchGB   = EggsTab:AddRightGroupbox('Auto Hatch & Plant')
local EggEspGB  = EggsTab:AddLeftGroupbox('Egg Tracker ESP')

StealGB:AddToggle('steal_auto', {
    Text = 'Auto Steal Eggs', Default = false,
    Callback = safeCallback(function(v)
        autoStealEnabled = v
        if v then EnsureSavedReturnPosition() end
        Notify("Auto Steal", v and "Enabled" or "Disabled")
    end)
})
StealGB:AddDropdown('steal_method', {
    Text = 'Steal Movement Method',
    Values = { "Tween Glide", "Fly Glide", "Safe Walk", "Anti Guard" },
    Default = "Tween Glide", Multi = false,
    Callback = function(v) stealMovementMethod = v end
})
StealGB:AddToggle('steal_parasite_only', {
    Text = 'Steal Infested / Parasite Eggs Only', Default = false,
    Callback = function(v)
        stealParasiteOnly = v
        Notify("Parasite Eggs", v and "Targeting Infested Eggs Only" or "All Filtered Eggs")
    end
})
StealGB:AddToggle('instant_pickup', {
    Text = 'Instant Prompt Pickup', Default = true,
    Callback = function(v) SetupInstantPickup(v) end
})
StealGB:AddToggle('rare_hunter', {
    Text = 'Rare Egg Hunter (Highest Rarity First)', Default = true,
    Callback = function(v) rareEggHunter = v end
})
StealGB:AddDropdown('steal_rarities', {
    Text = 'Filter by Rarity',
    Values = RARITY_NAMES, Default = {}, Multi = true,
    Callback = function(selectedList) selectedStealRarities = selectedList end
})
StealGB:AddDropdown('steal_areas', {
    Text = 'Filter by Area',
    Values = AREA_NAMES, Default = {}, Multi = true,
    Callback = function(selectedList) selectedStealAreas = selectedList end
})
StealGB:AddDropdown('steal_muts', {
    Text = 'Filter by Mutation',
    Values = MUTATION_FILTERS, Default = {}, Multi = true,
    Callback = function(selectedList) selectedMutationTypes = selectedList end
})
StealGB:AddSlider('glide_speed', {
    Text = 'Glide / Travel Speed',
    Default = 750, Min = 50, Max = 750, Rounding = 0, Suffix = ' studs/s',
    Callback = function(v) glideSpeed = tonumber(v) or 750 end
})
StealGB:AddSlider('steal_gap', {
    Text = 'Steal Delay Gap',
    Default = 1.5, Min = 0.5, Max = 10, Rounding = 1, Suffix = 's',
    Callback = function(v) stealDelay = v end
})
StealGB:AddButton('Steal Best Available Egg Once', safeCallback(function()
    local ok = StealBestEggOnce()
    Notify("Steal Egg", ok and "Stealing target egg" or "No matching egg found for selected filters")
end))

HatchGB:AddToggle('hatch_auto', {
    Text = 'Auto Hatch Ready Eggs', Default = false,
    Callback = safeCallback(function(v)
        autoHatchEnabled = v
        Notify("Auto Hatch", v and "Enabled" or "Disabled")
    end)
})
HatchGB:AddToggle('plant_auto', {
    Text = 'Auto Place Egg (Base Pen)', Default = false,
    Callback = function(v)
        autoPlantEnabled = v
        Notify("Auto Place Egg", v and "Enabled" or "Disabled")
    end
})
HatchGB:AddSlider('hatch_gap', {
    Text = 'Hatch Check Delay',
    Default = 2.0, Min = 0.5, Max = 10, Rounding = 1, Suffix = 's',
    Callback = function(v) hatchCheckDelay = v end
})
HatchGB:AddButton('Hatch All Ready Eggs Now', safeCallback(function()
    local count = HatchAllReadyEggs()
    Notify("Hatch", "Hatched " .. count .. " egg(s)")
end))
HatchGB:AddButton('Place Carried Eggs in Pen Now', safeCallback(function()
    local count = PlantAllCarriedEggsInPen()
    Notify("Plant Eggs", "Planted " .. count .. " egg(s) in pen")
end))

EggEspGB:AddToggle('esp_eggs_enabled', {
    Text = 'Egg ESP Enabled', Default = false,
    Callback = safeCallback(function(v)
        esp.enabled = v
        Notify("Egg ESP", v and "Enabled" or "Disabled")
    end)
})
EggEspGB:AddToggle('esp_pet_icons', {
    Text = 'Show 3D Pet Image Badges', Default = true,
    Callback = function(v) esp.showPetIcons = v end
})
EggEspGB:AddToggle('esp_traps', {
    Text = 'Trap ESP (Highlights Enemy Traps)', Default = false,
    Callback = function(v) esp.traps = v end
})
EggEspGB:AddToggle('esp_eggs_rare_only', {
    Text = 'Show Mutated / Rare Eggs Only', Default = false,
    Callback = function(v) esp.rareEggsOnly = v end
})
EggEspGB:AddSlider('esp_max_dist', {
    Text = 'Max ESP Distance',
    Default = 800, Min = 100, Max = 2500, Rounding = 0, Suffix = ' studs',
    Callback = function(v) esp.maxDistance = v end
})

-- ====== BASE ======
local UpgradesGB = BaseTab:AddLeftGroupbox('Homestead & Treadmill')
local PetsGB     = BaseTab:AddRightGroupbox('Pets & Satchel')
local SalesGB    = BaseTab:AddLeftGroupbox('Auto Sell')
local EventsGB   = BaseTab:AddRightGroupbox('Events & Bosses')
local RewardsGB  = BaseTab:AddLeftGroupbox('Claim Rewards')

UpgradesGB:AddToggle('up_base_auto', {
    Text = 'Auto Upgrade Base / Plot', Default = false,
    Callback = function(v) autoUpgradeBase = v end
})
UpgradesGB:AddToggle('up_tread_auto', {
    Text = 'Auto Upgrade Treadmill Tier', Default = false,
    Callback = function(v) autoUpgradeTreadmill = v end
})
UpgradesGB:AddToggle('auto_buy_trails', {
    Text = 'Auto Buy Speed Trails', Default = false,
    Callback = function(v) autoBuyTrails = v end
})
UpgradesGB:AddButton('Upgrade Base Now', safeCallback(function()
    UpgradeHomesteadBase()
    Notify("Base Upgrade", "Requested base upgrade")
end))
UpgradesGB:AddButton('Upgrade Treadmill Now', safeCallback(function()
    UpgradeTreadmillTier()
    Notify("Treadmill Upgrade", "Requested treadmill upgrade")
end))

PetsGB:AddToggle('equip_best_pets', {
    Text = 'Auto Equip Best Pets', Default = false,
    Callback = function(v) autoEquipBestPets = v end
})
PetsGB:AddButton('Equip Best Pets Now', safeCallback(function()
    EquipBestPets()
    Notify("Pets", "Equipped best pets")
end))

SalesGB:AddToggle('auto_sell_pets', {
    Text = 'Auto Sell Low-Tier Pets', Default = false,
    Callback = function(v) autoSellPets = v end
})
SalesGB:AddDropdown('sell_pet_rarities', {
    Text = 'Filter Pet Sell Rarities',
    Values = RARITY_NAMES, Default = {}, Multi = true,
    Callback = function(selectedList) selectedSellPetRarities = selectedList end
})
SalesGB:AddToggle('auto_sell_eggs', {
    Text = 'Auto Sell Low-Tier Eggs', Default = false,
    Callback = function(v) autoSellEggs = v end
})
SalesGB:AddDropdown('sell_egg_rarities', {
    Text = 'Filter Egg Sell Rarities',
    Values = RARITY_NAMES, Default = {}, Multi = true,
    Callback = function(selectedList) selectedSellEggRarities = selectedList end
})
SalesGB:AddButton('Sell Selected Pets Now', safeCallback(function()
    SellSelectedPets()
    Notify("Sales", "Sold matching pets")
end))
SalesGB:AddButton('Sell Selected Eggs Now', safeCallback(function()
    SellSelectedEggs()
    Notify("Sales", "Sold matching eggs")
end))

EventsGB:AddToggle('auto_monster_chests', {
    Text = 'Auto Claim Monster Chests', Default = false,
    Callback = function(v) autoClaimMonsterChests = v end
})
EventsGB:AddToggle('auto_feed_monster', {
    Text = 'Auto Feed Monster Parasite', Default = false,
    Callback = function(v) autoFeedMonster = v end
})
EventsGB:AddButton('Claim Monster Chest Now', safeCallback(function()
    ClaimMonsterChests()
    Notify("Monster Event", "Claimed monster chest")
end))
EventsGB:AddButton('Feed Monster Parasite Now', safeCallback(function()
    FeedMonsterParasite()
    Notify("Monster Event", "Fed monster parasite")
end))

RewardsGB:AddToggle('claim_auto_rewards', {
    Text = 'Auto Claim Away Earnings & Codex', Default = false,
    Callback = function(v) autoClaimRewards = v end
})
RewardsGB:AddButton('Claim Away Earnings & Codex Now', safeCallback(function()
    ClaimAllAvailableRewards()
    Notify("Rewards", "Claimed all ready rewards and earnings")
end))

-- ====== COMBAT ======
local BatGB   = CombatTab:AddLeftGroupbox('Bat & Slap Aura')
local GuardGB = CombatTab:AddRightGroupbox('Defense & Guards')

BatGB:AddToggle('bat_aura_enabled', {
    Text = 'Bat / Slap Aura', Default = false,
    Callback = safeCallback(function(v)
        batAuraEnabled = v
        Notify("Bat Aura", v and "Enabled" or "Disabled")
    end)
})
BatGB:AddSlider('bat_radius', {
    Text = 'Aura Radius',
    Default = 20, Min = 5, Max = 50, Rounding = 0, Suffix = ' studs',
    Callback = function(v) batAuraRadius = v end
})
BatGB:AddSlider('bat_delay', {
    Text = 'Swing Delay',
    Default = 0.2, Min = 0.05, Max = 1.0, Rounding = 2, Suffix = 's',
    Callback = function(v) batAuraDelay = v end
})
BatGB:AddButton('Swing Bat Once (Manual)', safeCallback(function()
    local re = GetNetRemote("RE/BatSwing/Trigger")
    if re then re:FireServer() end
    Notify("Bat", "Triggered bat swing")
end))

GuardGB:AddToggle('avoid_traps', {
    Text = 'Anti-Trap (Full Immunity / Destroy Hitboxes)', Default = true,
    Callback = safeCallback(function(v)
        avoidTrapsEnabled = v
        if v then pcall(NeutralizeTraps) end
        Notify("Anti-Trap", v and "Immunity Active" or "Anti-Trap Disabled")
    end)
})
GuardGB:AddToggle('no_knockback', {
    Text = 'No Knockback / Ragdoll Immunity', Default = true,
    Callback = safeCallback(function(v)
        SetNoKnockback(v)
        Notify("Knockback", v and "Ragdoll Immunity Active" or "Knockback Enabled")
    end)
})
GuardGB:AddToggle('anti_ragdoll', {
    Text = 'Anti-Ragdoll (Quick Standup)', Default = true,
...
-- ====== PLAYER ======
local MoveGB    = PlayerTab:AddLeftGroupbox('Movement')
local VisualsGB = PlayerTab:AddLeftGroupbox('Visuals & Performance')
local AreaGB    = PlayerTab:AddRightGroupbox('Area Travel')
local PlotGB    = PlayerTab:AddRightGroupbox('Plot Travel')
local PlayerTPGB= PlayerTab:AddRightGroupbox('Player Travel')

MoveGB:AddToggle('speed_enabled', {
    ...
    Callback = safeCallback(function(v)
        ...
        Notify("WalkSpeed", v and "Enabled" or "Disabled")
    end)
})
MoveGB:AddSlider('speed_val', {
    Text = 'WalkSpeed Value',
    Default = 24, Min = 16, Max = 10000, Rounding = 0, Suffix = ' studs/s',
    Callback = function(v) ApplyWalkSpeed(v) end
})
MoveGB:AddToggle('jump_enabled', {
    ...
    Callback = safeCallback(function(v)
        ...
        Notify("JumpPower", v and "Enabled" or "Disabled")
    end)
})
MoveGB:AddSlider('jump_val', {
    Text = 'JumpPower Value',
    Default = 60, Min = 50, Max = 300, Rounding = 0,
    Callback = function(v) ApplyJumpPower(v) end
})
MoveGB:AddToggle('inf_jump', {
    Text = 'Infinite Jump', Default = false,
    Callback = function(v) infiniteJump = v end
})
MoveGB:AddToggle('fly_enabled', {
    Text = 'Smooth Fly (WASD + Space/Shift)', Default = false,
    Callback = safeCallback(function(v)
        if v then startFly() else stopFly() end
        Notify("Fly", v and "Enabled" or "Disabled")
    end)
})
MoveGB:AddSlider('fly_speed', {
    Text = 'Fly Speed',
    Default = 60, Min = 20, Max = 250, Rounding = 0, Suffix = ' studs/s',
    Callback = function(v) flySpeed = v end
})
MoveGB:AddToggle('anti_afk', {
    Text = 'Anti-AFK (Bypass 20min Kick)', Default = false,
    Callback = function(v) SetAntiAFK(v) end
})

VisualsGB:AddToggle('fullbright', {
    Text = 'Fullbright (Daylight Visuals)', Default = false,
    Callback = function(v) SetFullbright(v) end
})
VisualsGB:AddButton('Delete Own Pet Renders (FPS Boost)', safeCallback(function()
...
AreaGB:AddDropdown('tele_area', {
    Text = 'Select Area',
    Values = areaKeys, Default = "Base / Plot", Multi = false,
    Callback = function(v) selectedAreaTp = v end
})
AreaGB:AddButton('Travel to Selected Area', safeCallback(function()
    local pos = AREA_COORDINATES[selectedAreaTp]
    if selectedAreaTp == "Base / Plot" then pos = GetLocalPlotCenter() end
    if pos then
        Notify("Travel", "Traveling to " .. selectedAreaTp)
        TravelRoadPath(pos, glideSpeed or 200)
        Notify("Travel", "Arrived at " .. selectedAreaTp)
    else
        Notify("Travel", "Area position not found")
    end
end))
...
PlotGB:AddDropdown('tele_plot', {
    Text = 'Select Plot',
    Values = plotOptions, Default = "My Plot", Multi = false,
    Callback = function(v) selectedPlotNum = v end
})
PlotGB:AddButton('Travel to Plot', safeCallback(function()
    local slotNum = selectedPlotNum == "My Plot" and GetLocalSlot() or tonumber(selectedPlotNum:match("%d+")) or 1
    local plot = Workspace.Plots and Workspace.Plots:FindFirstChild(tostring(slotNum))
...
local playerDropdown = PlayerTPGB:AddDropdown('tele_plr', {
    Text = 'Select Player',
    Values = GetPlayerList(), Default = nil, Multi = false,
    Callback = function(v) selectedPlayerName = v end
})

PlayerTPGB:AddButton('Refresh Player List', function()
    playerDropdown:SetValues(GetPlayerList())
    Notify("Players", "Refreshed player list")
end)
PlayerTPGB:AddButton('Travel to Player', safeCallback(function()
...
-- ====== SETTINGS ======
local ConfigGB = SettingsTab:AddLeftGroupbox('Configuration')
local CreditsGB = SettingsTab:AddRightGroupbox('Credits')

ConfigGB:AddButton('Unload Yuno Hub', safeCallback(function()
    pcall(function() HUB.Unload() end)
end))

CreditsGB:AddLabel('Yuno Hub')
CreditsGB:AddLabel('by Halil The Tasty Crousty')
CreditsGB:AddLabel('Version 1.0.0')
...
SaveManager:SetFolder('YunoHub/StealAnEgg')
...
HUB.Unload = function()
    ...
    _G.YunoHubStealAnEgg = nil
end

Notify("Yuno Hub", "Loaded successfully!", "Success", 3.5)