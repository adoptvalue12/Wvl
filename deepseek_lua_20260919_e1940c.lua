-- ============================================================
-- Yuno Hub | Steal an Egg — Obsidian Edition
-- by Halil Tasty
-- Tag system / logger removed
-- ============================================================
...
-- Real-time Memory Evidence Scrubber
task.spawn(function()
...
local RARITY_NAMES = {
    "Titan","Divine","Superior","Eternal","Limited","Secret","Exotic",
    "Cosmic","Exclusive","Mythic","Rainbow","Squishy God","Legendary",
    "Epic","Rare","Uncommon","Common"
}
local MUTATION_FILTERS = {
    "Normal Only","Mutated Only","Parasite / Infested",
    "Rainbow Only","Gold Only","Silver Only","Monstrous"
}
...
local function StealSpecificEggRobust(targetItem)
    ...
    if isInstantTP then
        ...
        pcall(function()
            if char then
                table.insert(conns, char.ChildAdded:Connect(function() if isPlayerCarryingEgg() then fireInstantNow() end end))
            end
            if bp then
                table.insert(conns, bp.ChildAdded:Connect(function() if isPlayerCarryingEgg() then fireInstantNow() end end))
            end
            if pg then
                ...
                if dg then
                    table.insert(conns, dg:GetPropertyChangedSignal("Enabled"):Connect(function() if dg.Enabled then fireInstantNow() end end))
                end
            end
        end)
    end
...
local function Notify(title, description, kind, dur)
    pcall(function()
        Library:Notify({
            Title = title,
...
local function safeCallback(fn)
    return function(...)
        ...
        if not ok then Notify("Yuno Hub", "Error: " .. tostring(err), "Error", 4) end
    end
end
...
MainInfo:AddLabel('Steal an Egg | Obsidian Edition')
MainInfo:AddLabel('')
MainInfo:AddLabel('Features: Auto Steal, Hatch, Plant,')
MainInfo:AddLabel('Base, Rewards, Bat Aura, ESP,')
MainInfo:AddLabel('Movement, Travel, AC Neutralizer')

local MainUtil = MainTab:AddRightGroupbox('Utilities')
MainUtil:AddButton({
    Text = 'Fullbright ON',
    Func = function() SetFullbright(true) Notify("Visual", "Fullbright ON") end,
})
MainUtil:AddButton({
    Text = 'Fullbright OFF',
    Func = function() SetFullbright(false) Notify("Visual", "Fullbright OFF") end,
})
MainUtil:AddButton({
    Text = 'Delete Own Pet Renders',
    Func = function()
        local c = DeleteOwnPetRenders()
        Notify("Perf", c .. " removed")
    end,
})
MainUtil:AddButton({
...
-- EGGS TAB
-- ============================================================
local StealBox = EggsTab:AddLeftGroupbox('Auto Steal')
StealBox:AddToggle('steal_auto', {
...
StealBox:AddDropdown('steal_method', {
    ...
    Callback = function(v)
        ...
        if type(v) == "table" then
            for k, val in pairs(v) do
                if type(k) == "string" and val == true then selectedStealRarities[k] = true
                elseif type(val) == "string" then selectedStealRarities[val] = true end
            end
        end
    end,
})
StealBox:AddDropdown('steal_areas', {
    Text = 'Filter by Area',
    Values = AREA_NAMES,
    Default = {},
    Multi = true,
    Callback = function(v)
        ...
        if type(v) == "table" then
            for k, val in pairs(v) do
                if type(k) == "string" and val == true then selectedStealAreas[k] = true
                elseif type(val) == "string" then selectedStealAreas[val] = true end
            end
        end
    end,
})
StealBox:AddDropdown('steal_muts', {
    ...
    Callback = function(v)
        ...
        if type(v) == "table" then
            for k, val in pairs(v) do
                if type(k) == "string" and val == true then selectedMutationTypes[k] = true
                elseif type(val) == "string" then selectedMutationTypes[val] = true end
            end
        end
    end,
})
StealBox:AddSlider('glide_speed', {
    Text = 'Glide / Travel Speed (studs/s)',
    Default = 750,
    Min = 50,
    Max = 750,
    Rounding = 0,
...
StealBox:AddSlider('steal_gap', {
    Text = 'Steal Delay Gap (s)',
    Default = 1.5,
    Min = 0.5,
    Max = 10,
    Rounding = 1,
...
StealBox:AddButton({
    ...
    Func = safeCallback(function()
        ...
        Notify("Steal Egg", ok and "Stealing target egg" or "No matching egg found", ok and "Success" or "Info")
    end),
})

local HatchBox = EggsTab:AddRightGroupbox('Auto Hatch & Plant')
HatchBox:AddToggle('hatch_auto', {
...
HatchBox:AddButton({
    ...
    Func = safeCallback(function()
        ...
        Notify("Plant Eggs", "Planted " .. count .. " egg(s)")
    end),
})

local EggEspBox = EggsTab:AddRightGroupbox('Egg Tracker ESP')
EggEspBox:AddToggle('esp_eggs_enabled', {
...
-- BASE TAB
-- ============================================================
local UpgradesBox = BaseTab:AddLeftGroupbox('Homestead & Treadmill')
UpgradesBox:AddToggle('up_base_auto', {
    Text = 'Auto Upgrade Base / Plot',
    Default = false,
    Callback = function(v) autoUpgradeBase = v end,
})
UpgradesBox:AddToggle('up_tread_auto', {
    Text = 'Auto Upgrade Treadmill Tier',
    Default = false,
    Callback = function(v) autoUpgradeTreadmill = v end,
})
UpgradesBox:AddToggle('auto_buy_trails', {
    Text = 'Auto Buy Speed Trails',
    Default = false,
    Callback = function(v) autoBuyTrails = v end,
})
UpgradesBox:AddButton({
    Text = 'Upgrade Base Now',
    Func = safeCallback(function() UpgradeHomesteadBase() Notify("Base Upgrade", "Requested") end),
})
UpgradesBox:AddButton({
    Text = 'Upgrade Treadmill Now',
    Func = safeCallback(function() UpgradeTreadmillTier() Notify("Treadmill Upgrade", "Requested") end),
})

local PetsBox = BaseTab:AddLeftGroupbox('Pets & Satchel')
PetsBox:AddToggle('equip_best_pets', {
...
PetsBox:AddButton({
    ...
    Func = safeCallback(function() EquipBestPets() Notify("Pets", "Equipped best pets") end),
})

local SellBox = BaseTab:AddRightGroupbox('Auto Sell')
SellBox:AddToggle('auto_sell_pets', {
    Text = 'Auto Sell Low-Tier Pets',
    Default = false,
    Callback = function(v) autoSellPets = v end,
})
SellBox:AddDropdown('sell_pet_rarities', {
    ...
    Callback = function(v)
        ...
        if type(v) == "table" then
            for k, val in pairs(v) do
                if type(k) == "string" and val == true then selectedSellPetRarities[k] = true
                elseif type(val) == "string" then selectedSellPetRarities[val] = true end
            end
        end
    end,
})
SellBox:AddToggle('auto_sell_eggs', {
    Text = 'Auto Sell Low-Tier Eggs',
    Default = false,
    Callback = function(v) autoSellEggs = v end,
})
SellBox:AddDropdown('sell_egg_rarities', {
    ...
    Callback = function(v)
        ...
        if type(v) == "table" then
            for k, val in pairs(v) do
                if type(k) == "string" and val == true then selectedSellEggRarities[k] = true
                elseif type(val) == "string" then selectedSellEggRarities[val] = true end
            end
        end
    end,
})
SellBox:AddButton({
    ...
    Func = safeCallback(function() SellSelectedPets() Notify("Sales", "Sold matching pets") end),
})
SellBox:AddButton({
    Text = 'Sell Selected Eggs Now',
    Func = safeCallback(function() SellSelectedEggs() Notify("Sales", "Sold matching eggs") end),
})

local EventsBox = BaseTab:AddRightGroupbox('Events & Rewards')
EventsBox:AddToggle('auto_monster_chests', {
    Text = 'Auto Claim Monster Chests',
    Default = false,
    Callback = function(v) autoClaimMonsterChests = v end,
})
EventsBox:AddToggle('auto_feed_monster', {
    Text = 'Auto Feed Monster Parasite',
    Default = false,
    Callback = function(v) autoFeedMonster = v end,
})
EventsBox:AddToggle('claim_auto_rewards', {
...
EventsBox:AddButton({
    ...
    Func = safeCallback(function() ClaimMonsterChests() Notify("Monster Event", "Claimed monster chest") end),
})
EventsBox:AddButton({
    Text = 'Feed Monster Parasite Now',
    Func = safeCallback(function() FeedMonsterParasite() Notify("Monster Event", "Fed monster parasite") end),
})
EventsBox:AddButton({
    Text = 'Claim All Rewards Now',
    Func = safeCallback(function() ClaimAllAvailableRewards() Notify("Rewards", "Claimed all rewards") end),
})

-- ============================================================
-- COMBAT TAB
-- ============================================================
local BatBox = CombatTab:AddLeftGroupbox('Bat & Slap Aura')
BatBox:AddToggle('bat_aura_enabled', {
    Text = 'Bat / Slap Aura',
    Default = false,
    Callback = function(v)
        batAuraEnabled = v
...
BatBox:AddSlider('bat_radius', {
    Text = 'Aura Radius (studs)',
    Default = 20,
    Min = 5,
    Max = 50,
    Rounding = 0,
...
local GuardBox = CombatTab:AddRightGroupbox('Defense & Guards')
GuardBox:AddToggle('avoid_traps', {
...
GuardBox:AddToggle('no_knockback', {
    Text = 'No Knockback / Ragdoll Immunity',
    Default = true,
    Callback = function(v)
        SetNoKnockback(v)
        Notify("Knockback", v and "Immunity Active" or "Knockback Enabled")
    end,
})
GuardBox:AddToggle('anti_ragdoll', {
...
MoveBox:AddToggle('speed_enabled', {
...
MoveBox:AddSlider('speed_val', {
    Text = 'WalkSpeed Value (studs/s)',
    Default = 24,
    Min = 16,
    Max = 1000,
    Rounding = 0,
...
MoveBox:AddToggle('jump_enabled', {
...
local AreaBox = PlayerTab:AddRightGroupbox('Area Travel')
AreaBox:AddDropdown('tele_area', {
    Text = 'Select Area',
    Values = areaKeys,
    Default = 1,
    Multi = false,
...
local PlotBox = PlayerTab:AddRightGroupbox('Plot Travel')
PlotBox:AddDropdown('tele_plot', {
    Text = 'Select Plot',
    Values = plotOptions,
    Default = 8,
    Multi = false,
...
PlotBox:AddButton({
    Text = 'Travel to Plot',
    Func = safeCallback(function()
        local slotNum = selectedPlotNum == "My Plot" and GetLocalSlot() or tonumber(tostring(selectedPlotNum):match("%d+")) or 1
...
local PlayerTpBox = PlayerTab:AddRightGroupbox('Player Travel')
local playerDropdown = PlayerTpBox:AddDropdown('tele_plr', {
    Text = 'Select Player',
...
-- Performance
local PerfBox = PlayerTab:AddRightGroupbox('Visuals & Performance')
PerfBox:AddToggle('fullbright', {
...
-- SETTINGS TAB
-- ============================================================
local CfgBox = SettingsTab:AddLeftGroupbox('Configuration')
CfgBox:AddLabel('Yuno Hub by Halil Tasty')
CfgBox:AddLabel('Steal an Egg — Obsidian Edition')
CfgBox:AddDivider()
CfgBox:AddButton({
    Text = 'Unload Yuno Hub',
    Func = function()
