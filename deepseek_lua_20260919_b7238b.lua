-- =====================================================================
-- Yuno Hub by Halil Tasty | Steal an Egg — FULL MERGED (6 scripts)
-- =====================================================================
...
local HUB = { conns = {}, drawings = {}, threads = {}, dead = false }
_G.YunoHub = HUB
...
do
    ...
    if not WindUI then
        warn("[Yuno Hub] WindUI failed to load")
        return
    end
end
...
do
    ...
    local parentOk = pcall(function() bgGui.Parent = (gethui and gethui()) or game:GetService("CoreGui") end)
    if not parentOk or not bgGui.Parent then
        pcall(function() bgGui.Parent = LP:WaitForChild("PlayerGui", 5) end)
    end
...
do
    ...
    if not ok or not result then
        warn("[Yuno Hub] Window creation failed: " .. tostring(result))
        return
    end
    Window = result
end
...
local function Notify(title, content, kind, dur)
    pcall(function() Window:Notify({ Title = title, Content = content, Icon = "info", Duration = dur or 2.5 }) end)
end
...
local antiSlideRuntime = true
local antiKnockbackRuntime = true
local antiRagdollRuntime = true
local avoidTrapsRuntime = true
local instantPickupRuntime = true
...
track(RunService.Heartbeat:Connect(function()
    if HUB.dead or not antiRagdollRuntime then return end
    local hum = findHum()
...
EggsTab:Dropdown({ Flag = "steal_method", Title = "Movement Method", Desc = "Travel method",
...
EggsTab:Slider({ Flag = "glide_speed", Title = "Glide Speed", Desc = "Studs/s", Step = 10,
    Value = { Min = 50, Max = 750, Default = 750 },
    Callback = function(v) glideSpeed = v end })
EggsTab:Slider({ Flag = "steal_gap", Title = "Steal Delay", Desc = "Seconds between steals", Step = 0.1,
...
EggsTab:Button({ Title = "Hatch All Ready Now", Justify = "Center",
    Callback = function() local c = HatchAllReadyEggs() Notify("Hatch", c .. " hatched") end })
EggsTab:Button({ Title = "Plant All In Pen Now", Justify = "Center",
    Callback = function() local c = PlantAllCarriedEggsInPen() Notify("Plant", c .. " planted") end })
...
PetsTab:Button({ Title = "Clear Spawned Pets", Justify = "Center",
    Callback = function() clearAllSpawnedPets() Notify("Pets", "Cleared") end })
if #CATALOG > 0 then
    local petNames = {}
    for _, p in ipairs(CATALOG) do table.insert(petNames, p.display) end
    PetsTab:Dropdown({ Flag = "pet_pick", Title = "Spawn Pet (pick)", Values = petNames, Value = petNames[1],
        Callback = function(sel)
            for _, p in ipairs(CATALOG) do
                if p.display == sel.Title then
...
BaseTab:Button({ Title = "Claim Monster Chest Now", Justify = "Center", Callback = function() ClaimMonsterChests() Notify("Monster", "Claimed") end })
BaseTab:Button({ Title = "Feed Monster Parasite Now", Justify = "Center", Callback = function() FeedMonsterParasite() Notify("Monster", "Fed") end })
...
BaseTab:Button({ Title = "Sell Low-Tier Pets Now", Justify = "Center", Callback = function() SellSelectedPets(nil) Notify("Sell", "Sold") end })
BaseTab:Button({ Title = "Sell Low-Tier Eggs Now", Justify = "Center", Callback = function() SellSelectedEggs() Notify("Sell", "Sold") end })
...
CombatTab:Button({ Title = "Swing Bat Once", Justify = "Center",
    Callback = function()
        local re = GetNetRemote("RE/BatSwing/Trigger")
        if re then pcall(function() re:FireServer() end) end
        Notify("Bat", "Swing")
    end })
CombatTab:Toggle({ Flag = "anti_ragdoll", Title = "Anti-Ragdoll", Default = true,
    Callback = function(v) antiRagdollEnabled = v antiRagdollRuntime = v end })
...
CombatTab:Toggle({ Flag = "anti_kb", Title = "Anti-Knockback", Default = true,
    Callback = function(v) antiKnockbackEnabled = v antiKnockbackRuntime = v end })
CombatTab:Toggle({ Flag = "avoid_traps", Title = "Anti-Trap", Default = true,
    Callback = function(v) avoidTrapsEnabled = v if v then pcall(NeutralizeTraps) end end })
...
table.sort(areaKeys)
TravelTab:Dropdown({ Flag = "tp_area", Title = "Area", Values = areaKeys, Value = "Base / Plot",
    Callback = function(s) selectedAreaTP = s.Title end })
TravelTab:Button({ Title = "Travel to Area", Justify = "Center",
    Callback = function()
        local pos = AREA_COORDINATES[selectedAreaTP]
        if pos then
            Notify("Travel", "To " .. selectedAreaTP)
            TravelRoadPath(pos, glideSpeed)
            Notify("Travel", "Arrived")
        end
    end })
local selectedPlotNum = "My Plot"
local plotOptions = { "Plot 1","Plot 2","Plot 3","Plot 4","Plot 5","Plot 6","Plot 7","My Plot" }
TravelTab:Dropdown({ Flag = "tp_plot", Title = "Plot", Values = plotOptions, Value = "My Plot",
    Callback = function(s) selectedPlotNum = s.Title end })
...
TravelTab:Dropdown({ Flag = "tp_plr", Title = "Player", Values = getPlayerList(), Value = nil,
    Callback = function(s) selectedPlayerName = s.Title end })
TravelTab:Button({ Title = "Refresh Player List", Justify = "Center",
    Callback = function() Notify("Players", "Refreshed") end })
TravelTab:Button({ Title = "Travel to Player", Justify = "Center",
    Callback = function()
        ...
        if hrp then
            ...
            Notify("Travel", "Arrived at " .. selectedPlayerName)
        end
    end })
...
TravelTab:Button({ Title = "Save Current Waypoint", Justify = "Center",
    Callback = function()
        ...
        if r then
            ...
            Notify("Waypoint", "Saved: " .. wpInput)
        end
    end })
for i, wp in ipairs(waypoints) do
    TravelTab:Button({
        Title = wp.name .. " (" .. string.format("%.0f,%.0f,%.0f", wp.position.X, wp.position.Y, wp.position.Z) .. ")",
        Justify = "Center",
        Callback = function() teleportToWaypoint(i) end,
    })
end

-- Auto Hook Tab
local HookTab = Section:Tab({ Title = "Auto Hook", Icon = "anchor" })
HookTab:Toggle({ Flag = "hook_v1", Title = "Auto Hook v1 (Base drop)", Default = false,
    Callback = function(v) hookV1Enabled = v if v then hookV2Enabled = false end end })
HookTab:Toggle({ Flag = "hook_v2", Title = "Auto Hook v2 (Forest drop)", Default = false,
    Callback = function(v) hookV2Enabled = v if v then hookV1Enabled = false end end })
...
PerfTab:Button({ Title = "Delete Own Pet Renders", Justify = "Center",
    Callback = function() local c = DeleteOwnPetRenders() Notify("Perf", c .. " removed") end })
...
ServerTab:Button({ Title = "Hop Now", Justify = "Center",
    Callback = function()
        ...
        Notify("Hop", ok and "Teleporting..." or ("Failed: " .. tostring(err)))
    end })
...
SettingsTab:Keybind({ Flag = "ui_key", Title = "Toggle UI Key", Value = "RightControl",
    Callback = function(k) pcall(function() Window:SetToggleKey(Enum.KeyCode[k]) end) end })
SettingsTab:Button({ Title = "Unload Yuno Hub", Justify = "Center",
    Callback = function() pcall(function() HUB.Unload() end) end })
SettingsTab:Section({ Title = "Yuno Hub by Halil Tasty | 6 scripts merged", TextSize = 13 })
